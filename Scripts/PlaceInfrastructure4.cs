...
public class PlaceInfrastructure4 : MonoBehaviour {

	public GameObject infra5;
	GameObject newInfra1;
	Vector3 amountInfra1;
	int contInfra1;
	public bool flagInfra5;
	public PlaceInfrastructure3 scriptinfra4;
	...
	void Start ()
	{
		...
		scriptinfra4 = gameObject.GetComponent<PlaceInfrastructure3> ();
		flagInfra5 = false;
		contInfra1 = 0;
		amountInfra1 = new Vector3 (-0.5f, 1.5f, 10f);
	}

	// Update is called once per frame
	void Update ()
	{
		if(!flagInfra5 && scriptinfra4.flagInfra4)
		{
			if(SixenseInput.Controllers[0].GetButton(SixenseButtons.BUMPER) && SixenseInput.Controllers[1].GetButton(SixenseButtons.BUMPER))
			{
				...
				if(contInfra1 == 1)
				{
					newInfra1 = (GameObject)Instantiate (infra5, gameObject.transform.position + amountInfra1, Quaternion.identity);
